...
def n_gram_words():
    ...
    with open("../Dataset/Books/3_gram_words_allBook.json", 'w') as f:
        new_words = []
        for nb in book_list:
            print(nb)
            words_book_counter = []
        ...
        for i in range(len(new_words) - 2):
            words_3_gram.append(new_words[i] + ' ' + new_words[i + 1] + ' ' + new_words[i + 2])
        ...
        for word in words_3_gram:
            ...
            if b1 + b2 + b3 < 2 and not b3 and word1 != 'said':
...
def _3_gram_book(book):
    book_list = [book]
    with open('../Dataset/Books/stop_words_english.txt', 'r') as stop_words_english:
        stop_words = stop_words_english.read().splitlines()
    with open("../Dataset/Books/3_gram_words_book"+ book + ".json", 'w') as f:
        new_words = []
        for nb in book_list:
            print(nb)
            words_book_counter = []
...
def book_specification(book):
    all_words = pd.read_json('../Dataset/Books/3_gram_words_allBook.json')
    words_book = pd.read_json('../Dataset/Books/3_gram_words_book'+book+'.json')
    reduce_words_book = []
    ...
    with open("../Dataset/Books/3_gram_words_specific_book"+book+'.json', 'w') as f:
