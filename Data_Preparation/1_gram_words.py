...
def n_gram_words():
    ...
    with open("../Dataset/Books/1_gram/1_gram_words_allBook.json", 'w') as f:
        new_words = []
        for nb in book_list:
            print(nb)
            words_book_counter = []
            ...
            with open('../Dataset/Books/Book'+nb+'.txt', 'r') as book:
                ...
                allLines += lines
            words = [w for segments in allLines 
                                if (not segments.startswith('Page | ') and not segments.endswith('Rowling ')) 
                                for w in segments.split() if w.lower() not in stop_words]
        ...
        words_1_gram_without_stop_word = []
        for word in new_words:
            if word not in stop_words:
                words_1_gram_without_stop_word.append(word)
        wordsCounter = Counter(words_1_gram_without_stop_word)
...
def _1_gram_book(book):
    book_list = [book]
    with open('../Dataset/Books/stop_words_english.txt', 'r') as stop_words_english:
        stop_words = stop_words_english.read().splitlines()
    with open("../Dataset/Books/1_gram/1_gram_words_book"+ book + ".json", 'w') as f:
        new_words = []
        for nb in book_list:
            print(nb)
            words_book_counter = []
...
def book_specification(book):
    all_words = pd.read_json('../Dataset/Books/1_gram/1_gram_words_allBook.json')
    words_book = pd.read_json('../Dataset/Books/1_gram/1_gram_words_book'+book+'.json')
    reduce_words_book = []
    for i, row in words_book.iterrows():
        if not row['Words'].lower() in all_words[:200]['Words'].values:
